...
public class DAPValue extends XNamedValue {
    ...
    private static final Logger LOG = Logger.getInstance(DAPValue.class);
    @NotNull
    private final DSPThread process;
    @NotNull
    private final Variable variable;
    @Nullable
    private final Icon icon;

    public DAPValue(DSPThread process, @NotNull Variable variable, @Nullable Icon icon) {
    ...
    public void computeChildren(@NotNull XCompositeNode node) {
        try {
            ...
            VariablesResponse variableResp = process.client.getDebugProtocolServer().variables(varArgs).get();
            for (Variable variable : variableResp.getVariables()) {
                list.add(variable.getName(), new DAPValue(process, variable, getIconFor(variable)));
            }
    ...
    public void computeSourcePosition(@NotNull XNavigatable navigatable) {
        readActionInPooledThread(new Runnable() {
            ...
            private XSourcePosition findPosition() {
                XDebugSession debugSession = process.client.getSession();
