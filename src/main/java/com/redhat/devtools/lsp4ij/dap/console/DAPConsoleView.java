...
public class DAPConsoleView extends ConsoleViewImpl {
    ...
    // Equivalent of the anonymous NotStartedStated subclass in Kotlin
    ...
    public static @Nullable String getApplicableText(@NotNull String text, @NotNull ConsoleViewContentType contentType) {
        ...
        // At this
        if (text.startsWith("Content-Length:") || text.equals("\n")) {
            // DAP traces from stdio
            return null;
        }
        if (text.startsWith("{")) {
            ...
            if (s != null && (s.startsWith("Content-Length:") || s.equals("\n"))) {
                return null;
            }
            return s;
        }
    ...
    private static int findLastCompleteJsonBlockEnd(@NotNull String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (c == '{') {
                ...
                depth--;
                if (depth == 0) {
                    // Fin d'un bloc JSON complet, mémoriser la position
                    lastValidEnd = i;
                } else if (depth < 0) {
                    // Accolade fermante sans ouverture correspondante → reset
                    depth = 0;
                }
            }
        }
